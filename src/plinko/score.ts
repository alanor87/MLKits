...
const outputs: Array<DropType> = [];

function onScoreUpdate(dropPosition: number, bounciness: number, size: number, bucketLabel: number) {
...
function splitDataset(data: any, testCount: number) {
...
function knn(data:  Array<DropType>, point: DropType, k: number) {
  const res = Object.entries({
    ...[...data]
      .map((row) => {
        const a = [distance(row.slice(0, -1), point), row.at(-1) as number];
        return a;
      })
      .sort((a, b) => a[0] - b[0])
      .slice(0, k)
...
function runAnalysis() {
  ...
  for (let feature = 0; feature < 3; feature += 1) {
    const data = outputs.map(row => [row[feature], row.at(-1) as number]);
    const [testSet, trainingSet] = splitDataset(minMax(data, 1), testSetSize);
    const accuracy =
      (testSet.filter((testPoint) => {
...
function minMax(data: number[][], featureCount: number) {
  ...
  for (let i = 0; i < featureCount; i += 1) {
    ...
    const max = sortedColumn.at(-1) as number;
